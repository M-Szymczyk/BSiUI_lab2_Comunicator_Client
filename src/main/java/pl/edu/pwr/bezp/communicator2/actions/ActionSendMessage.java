...
public class ActionSendMessage extends AbstractAction{
    public ActionSendMessage(SocketsConnectionLayer connectionLayer, AES aes, BodyCreator bodyCreator) {
        super(connectionLayer, aes, bodyCreator);
    }

    @Override
    public RespAbstract run(CommunicatorClient communicatorClient) {
        return new RespAbstract(communication(new RequestData("message", bodyCreator.fillMessageBody(communicatorClient.getUserNames())))) {
        };
    }
}
